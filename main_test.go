...
func TestMultilineClipboardRoundTrip(t *testing.T) {
	h := &sqliteHistory{path: t.TempDir() + "/test.db"}
	if err := h.init(); err != nil {
		t.Fatal(err)
	}
	...
	body := strings.NewReader(`{"text":"line1\nline2\nline3","source":"mobile-web"}`)
	req := httptest.NewRequest(http.MethodPost, "/api/clipboard", body)
	rr := httptest.NewRecorder()
	a.handleClipboard(rr, req)
	...
	a.handleClipboard(getRR, getReq)
	...
	if got.Text != "line1\nline2\nline3" {
		t.Fatalf("unexpected multiline text: %q", got.Text)
	}
}
